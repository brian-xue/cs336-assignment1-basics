...
def run_linear(
    ...
    from cs336_basics.modules import LinearModule
    linear = LinearModule(in_features=d_in, out_features=d_out, device=in_features.device, dtype=in_features.dtype)
    linear.load_state_dict({"weight": weights.to(device=in_features.device, dtype=in_features.dtype)})
...
def run_embedding(
    ...
    from cs336_basics.modules import EmbeddingModule
    embedding = EmbeddingModule(num_embeddings=vocab_size, embedding_dim=d_model, device=token_ids.device, dtype=weights.dtype)
    embedding.load_state_dict({"embedding": weights.to(device=token_ids.device, dtype=weights.dtype)})
...
def run_swiglu(
    ...
    from cs336_basics.modules import SwiGLUModule
    swiglu = SwiGLUModule(d_model=d_model, d_ff=d_ff, device=in_features.device, dtype=in_features.dtype)
    swiglu.load_state_dict({
        "W1.weight": w1_weight.to(device=in_features.device, dtype=in_features.dtype),
        "W2.weight": w2_weight.to(device=in_features.device, dtype=in_features.dtype),
        "W3.weight": w3_weight.to(device=in_features.device, dtype=in_features.dtype),
    })
...
def run_scaled_dot_product_attention(
    ...
    """
    from cs336_basics.modules import dot_product_attention
...
def run_multihead_self_attention(
    ...
    """
    from cs336_basics.modules import MultiheadSelfAttentionModule
    mha = MultiheadSelfAttentionModule(d_model=d_model, num_heads=num_heads)
    qkv_weights = torch.cat([q_proj_weight, k_proj_weight, v_proj_weight], dim=0)

    mha.load_state_dict({"qkv_proj.weight": qkv_weights.to(device=in_features.device, dtype=in_features.dtype),
                        "o_proj.weight": o_proj_weight.to(device=in_features.device, dtype=in_features.dtype)})
...
def run_multihead_self_attention_with_rope(
    ...
    """
    from cs336_basics.modules import MultiheadSelfAttentionModule
    mha = MultiheadSelfAttentionModule(d_model=d_model, num_heads=num_heads, max_seq_len=max_seq_len, theta=theta)
...
def run_rope(
    ...
    """
    from cs336_basics.modules import RoPEModule
    rope = RoPEModule(theta=theta, d_k=d_k, max_seq_len=max_seq_len, device=in_query_or_key.device, dtype=in_query_or_key.dtype)
...
def run_rmsnorm(
    ...
    """
    from cs336_basics.modules import RMSNormModule
    rmsnorm = RMSNormModule(d_model=d_model, eps=eps, device=in_features.device, dtype=in_features.dtype)
